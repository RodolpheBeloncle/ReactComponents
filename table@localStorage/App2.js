...
function App2() {
 ...
 // get localstorage data + components check state with useEffect
    ...
    const getLocalStorage = () => {
        ...
        let allStoredProducts = localStorage.getItem("productList2")
        
       
        if (allStoredProducts){
         
          return (allStoredProducts = JSON.parse(localStorage.getItem("productList2")))
     ...
     const handleProductQuantityChange = (id, newQuantity) => {
      ...
      if (quantity === 0) {
        if (window.confirm('Etes vous sûr de vouloir supprimer cet item ?')) {
          ...
          localStorage.setItem("productList2",JSON.stringify(productList.filter((p) => p.id !== id)))
        }
        ...
        localStorage.setItem("productList2",JSON.stringify(productList.map((product) => product.id === id ? { ...product, quantity } : product)))
    ...
    const handleOnChangeCheckBox = (id,newState) => {
        ...
        if (state === false){
          ...
          localStorage.setItem("productList2",JSON.stringify(productList.map((product) => product.id === id ? { ...product, state:true} : product)))
        ...
        localStorage.setItem("productList2",JSON.stringify(productList.map((product) => product.id === id ? { ...product, state:false} : product)))
    ...
    const handleAddProduct = (event) => {
      ...
      localStorage.setItem("productList2",JSON.stringify(productList.concat([
      {
        id: uuidv4(),
        name: newProductName,
        price: parseFloat(newProductPrice),
